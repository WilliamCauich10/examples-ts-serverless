...
export class UsuarioManager {
    ...
    getUsuarios(userId?: string) {
      ...
      try {
        // query
        const query = instance('tutorials').select('id', 'title', 'description')
    ...
    postUsuario(data: any) {
      const instance = this.dbManager.getInstance()
      try {
        return instance('tutorials').insert(data)
      } catch (err) {
        console.log(err)
      }
    }
    ...
    updateUsuario(userId: string, body: any) {
      const instance = this.dbManager.getInstance()
      try {
        return instance('tutorials').where('id', userId).update(body)
      } catch (err) {
        console.log(err)
      }
    }
    ...
    deleteUsuario(userId: string) {
      const instance = this.dbManager.getInstance()
      try {
        return instance('tutorials').where('id', userId).delete()
      } catch (err) {
        console.log(err)
      }
    }
